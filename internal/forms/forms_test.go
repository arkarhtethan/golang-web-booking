...
func TestForm_MinLength(t *testing.T) {
	...
	if form.Valid() {
		t.Error("form show min length for non existance field")
	}
	...
	if form.Valid() {
		t.Error("form show min length of 100 when data is shorter")
	}
	postedData = url.Values{}
	...
	if !form.Valid() {
		t.Error("show minlength of 1 is not met when it is.")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("x")
	if form.Valid() {
		t.Error("form show email for non existance field")
	}
	...
	postedData = url.Values{}
	postedData.Add("another_field", "asdfadfkj")
	form = New(postedData)

	form.IsEmail("another_field")
	if form.Valid() {
		t.Error("got valid for nivalid email.")
	}
}
